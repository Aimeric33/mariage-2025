...
SimpleForm.setup do |config|
  ...
  config.button_class = 'my-2 bg-primary-600 hover:bg-primary-500 text-white font-bold py-2 px-4 rounded'
  ...
  config.label_class = 'mb-2 font-medium text-gray-800'
  ...
  config.wrappers :vertical_form, tag: 'div', class: 'mb-4' do |b|
    ...
    b.use :input, class: 'shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 bg-white focus:outline-none focus:ring-0 focus:border-primary-600 text-gray-800 leading-6 transition-colors duration-200 ease-in-out', error_class: 'border-red-500', valid_class: 'border-green-400'
  ...
  config.wrappers :vertical_form_select, tag: 'div', class: 'mb-4' do |b|
    ...
    b.optional :readonly
    b.use :label, class: 'block', error_class: 'text-red-500'
    b.use :input, class: 'shadow border border-gray-300 rounded w-full bg-white focus:outline-none focus:ring-0 focus:border-primary-600 text-gray-800 leading-6 transition-colors duration-200 ease-in-out', error_class: 'border-red-500', valid_class: 'border-green-400'
    b.use :full_error, wrap_with: { tag: 'p', class: 'mt-2 text-red-500 text-xs italic' }
    b.use :hint, wrap_with: { tag: 'p', class: 'mt-2 text-grey-700 text-xs italic' }
  end
  ...
  config.wrappers :vertical_collection, item_wrapper_class: 'flex items-center', item_label_class: 'my-1 ml-3 block text-gray-800', tag: 'div', class: 'my-4' do |b|
    b.use :html5
    b.optional :readonly
    b.wrapper :legend_tag, tag: 'legend', class: 'mt-4 mb-2 font-bold text-primary-600', error_class: 'text-red-500' do |ba|
      ba.use :label_text
    end
    b.use :input, class: 'focus:ring-2 focus:ring-primary-500 ring-offset-2 h-4 w-4 text-primary-600 border-gray-300 rounded', error_class: 'text-red-500', valid_class: 'text-green-400'
  ...
  config.wrappers :vertical_multi_select, tag: 'div', class: 'my-4', error_class: 'f', valid_class: '' do |b|
    ...
    b.wrapper tag: 'div', class: 'inline-flex space-x-1' do |ba|
      ...
      ba.use :input, class: 'flex w-auto w-auto shadow appearance-none border border-gray-300 rounded w-full p-2 bg-white focus:outline-none focus:border-blue-500 text-gray-800 leading-4 transition-colors duration-200 ease-in-out'
    end
    b.use :full_error, wrap_with: { tag: 'p', class: 'mt-2 text-red-500 text-xs italic' }
    b.use :hint, wrap_with: { tag: 'p', class: 'mt-2 text-grey-700 text-xs italic' }
  end
